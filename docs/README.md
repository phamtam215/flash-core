...
| **Sơ đồ tương tác để trình bày** (zoom, tô đường đi, xuất ảnh) — nội dung bám theo các chủ sở hữu ở bảng này | [`so-do/`](html/so-do/index.html) |
...
└── html/                  ★ BẢN ĐỌC BẰNG TRÌNH DUYỆT — một thư mục duy nhất
    ...
    └── so-do/               5 sơ đồ tương tác (Archify) + index.html mục lục;
                              mỗi .html có .json nguồn cạnh nó
```
