...
| **Benchmark báo xanh trong khi không đo được gì** | Ngưỡng **PASS ở giá trị 0**. Thêm `CsrfGuard` ở Phase 7 làm mọi request của k6 trả 403; chúng rơi vào nhãn `errors_4xx_other` mà không ngưỡng nào nhìn tới, nên `errors_5xx: count==0` PASS (0 lỗi) và `orders_created: count<=100` cũng PASS (0 ≤ 100). Bằng chứng oversell = 0 **im lặng biến mất** | Thêm vế chặn dưới: `orders_created: ['count<=100', 'count>0']` và `errors_4xx_other: ['count==0']`. Luật chung: **một ngưỡng PASS ở giá trị 0 thì không phải ngưỡng** — luôn hỏi "cái này có đỏ khi hệ thống không chạy gì không?" |
