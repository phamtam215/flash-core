...
- [x] Báo cáo benchmark k6 so sánh 3 chiến lược + kết luận khi nào dùng cái nào — test #16, 1.000 VU × 4 cấu hình, oversell = 0 và 5xx = 0 ở cả bốn. Số đo + ba kết quả **ngược trực giác** (pessimistic nhanh nhất khi 900/1.000 request là "hết hàng"; pool 50 chậm hơn pool 10; Redis chưa nhanh hơn vì Phase 3 còn ghi DB đồng bộ) ở [spec Phase 3 §Bằng chứng test #16](specs/phase3-order-concurrency.md) và [tech-playbook §Phase 3](tech-playbook.md)
...
- [x] ~10 ADR — hiện **14** (`docs/adr/001`–`011`). Ba cái cuối ghi lại quyết định đã áp dụng từ lâu mà chưa có ADR: [009 CSRF double-submit có ký](adr/009-csrf-double-submit-co-ky.md), [010 keyset pagination thay OFFSET](adr/010-keyset-pagination-thay-offset.md), [011 Idempotency-Key lấy UNIQUE của DB làm trọng tài](adr/011-idempotency-key-lay-db-lam-trong-tai.md). Phase 7 thêm ba: [012 worker chạy một lượt trên Cloud Run](adr/012-worker-tren-cloud-run.md), [013 pool nhỏ trên serverless](adr/013-pool-nho-tren-serverless.md), [014 Workload Identity Federation](adr/014-workload-identity-federation.md)
...
- [x] Payment webhook: verify chữ ký, idempotent, test webhook trùng & đến muộn — HMAC-SHA256 trên **raw body** + `timingSafeEqual`, dấu thời gian nằm trong payload đã ký để chống replay. Test 10–15 ở `test/async-payment.e2e-spec.ts`: chữ ký sai (11), body sửa sau khi ký (11b), lệch 10 phút (12), **cùng `eventId` gửi 2 lần** (13), tới đơn đã `CANCELLED` (14), lệch tiền (15)
- [~] API live trên Cloud Run, CI/CD tự động — **CI/CD xong** ([`deploy.yml`](../.github/workflows/deploy.yml): build → migrate → API → worker job → kiểm `/ready`), CI giờ chạy cả integration test trên Postgres/Redis thật. **Chưa deploy thật** vì cần tài khoản GCP + Secret Manager + budget alert — ranh giới việc nào máy làm / việc nào Tâm làm ở [spec Phase 7](specs/phase7-deploy-gcp.md)
